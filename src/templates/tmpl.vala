...
void o(int x){stdout.putc((char)x);}

void main(string[]n){
  var g="<QCImage/>";
  int[]r=new int[99];
  int i=<QCConst>entrypoint</QCConst>,p=0,a=0,c=0,d=0,s=8,l=<QCConst>image_len</QCConst>;
  // Put the name (the default key if no argument) into r0..; n[0] is the executable, so use n[1]
  // n is argv itself; argv[argc] is guaranteed NULL, so ?? catches the no-argument case
  var z=n[1]??"vala";
  for(;c<z.length;c++)
    r[c%8]=z[c];
  while(i<l){
